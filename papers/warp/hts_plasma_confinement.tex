...
\centering
\vspace{1cm}
\textbf{Framework Architecture Diagram}\\[0.3cm]
\textit{[Integrated HTS-Plasma system:]}\\[0.2cm]
\textbf{1. HTS Magnetic System} $\rightarrow$ \textbf{2. Plasma Simulation}\\[0.1cm]
$\downarrow$ \hspace{1.5cm} $\downarrow$\\[0.1cm]
\textbf{5. Data Acquisition} $\leftarrow$ \textbf{3. Thermal Management}\\[0.1cm]
$\uparrow$ \hspace{1.5cm} $\downarrow$\\[0.1cm]
\textbf{4. Safety \& Control Systems}\\[0.2cm]
\textit{Multi-physics framework.}
\vspace{1cm}
\end{minipage}}
...
\subsubsection{HTS Magnet System Validation}

We validate the HTS magnet design using the framework implemented in \texttt{src/hts/}. System-level validation (environment, dependencies, hardware) is documented in \texttt{validation\_results.json}, while detailed physics validation (23 tests including paper benchmarks, physical reasonableness, unit consistency, and uncertainty quantification) is documented in \texttt{notebooks/validation\_report.json}.

\textbf{Validated REBCO Magnet Performance}:

\begin{itemize}
\item \textbf{Baseline Configuration (2.1T)}: Achieved 0.01\% magnetic ripple with 1171A operating current and 400 turns, validated against paper benchmarks with <1\% tolerance (\texttt{src/hts/validation\_framework.py})

\item \textbf{High-Field Configuration (7.07T)}: Demonstrated 0.16\% magnetic ripple with 1800A current using 89-tape REBCO design at 15K operating temperature, providing 74.5K thermal margin to critical temperature (\texttt{src/hts/high\_field\_scaling.py})

\item \textbf{Thermal Analysis}: Kim model implementation (\texttt{src/hts/materials.py}) validates critical current density $J_c(T,B)$ relationships with liquid nitrogen cooling at 77K

\item \textbf{Mechanical Stress Validation}: Maxwell stress tensor calculations (\texttt{src/hts/mechanical\_reinforcement.py}) verify hoop stress reduction from 175 MPa (baseline) to 35 MPa (reinforced) using composite reinforcement strategies

\item \textbf{FEA Integration}: Unified interface (\texttt{src/hts/fea.py}) supports both COMSOL and FEniCSx solvers with <0.1\% cross-validation error for electromagnetic field calculations
\end{itemize}

\textbf{Computational Validation Framework}:

\begin{itemize}
\item \textbf{Paper Benchmark Reproduction}: 23 validation tests executed with success rate of 23/23 (see \texttt{notebooks/validation\_report.json}):
   \begin{itemize}
   \item Paper benchmarks: 6/6 tests passed
   \item Physical reasonableness: 12/12 tests passed  
   \item Unit consistency: 4/4 tests passed
   \item Numerical convergence: 1/1 tests passed
   \end{itemize}

\item \textbf{Uncertainty Quantification}: Monte Carlo sensitivity analysis (\texttt{src/hts/validation\_framework.py}) quantifies:
   \begin{itemize}
   \item Magnetic field uncertainty: $\pm 0.056$~T (2.7\% relative) from current, radius, and geometry variations
   \item Temperature margin uncertainty: $\pm 3.4$~K (5.9\% relative) from critical temperature and heating variations
   \item Critical current uncertainty: $\pm 6.0$~A/$\mu$m relative contribution from temperature and field dependencies
   \end{itemize}
\end{itemize}
...
\begin{tabular}{@{}lcccc@{}}
\toprule
Parameter & ITER & JET & W7-X & \textbf{This Work} \\
\midrule
